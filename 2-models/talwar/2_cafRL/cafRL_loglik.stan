// T-cafRL Model with all free parameters ----------------------------------
...
functions {
  ...
  real update_theta(int stage, int stimulus, vector f, real alpha, real beta, real theta) {
    ...
    real G; // Gradient 
    real inv_logit_theta; // Theta, dimension weight 
    real theta_updated; // Theta updated
...
data {
  ...
  real alpha_sd;
  real<lower=0> beta_alpha; // Inverse temperature
...
parameters {
  ...
  real<lower=0.0000001> beta; // Inverse temperature, defined on range 0-inf 
...
generated quantities {
  ...
  for (t in 1:n_trials) {
    ...
    f_updated = update_weights(stage[t], stimulus[t], f, alpha_prior_log, beta_prior, theta);
    if (stage[t] > 2) {
      theta_updated = update_theta(stage[t], stimulus[t], f, alpha_prior_log, beta_prior, theta);
    }
  }
