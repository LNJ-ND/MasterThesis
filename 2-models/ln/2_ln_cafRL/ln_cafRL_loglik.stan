// LN-cafRL Model with all free parameters ----------------------------------
...
functions {
  ...
  vector update_weights(int stage, int stimulus, vector f, real alpha, real beta, real theta) {
    ...
    vector[4] f_updated; // updated features
    
    // Sigmoid, put theta on 0-1 scale 
    ...
    // Return udpated features
    return f_updated;
    
  }
    
  // Function to update the dimension weight theta
...
data {
  ...
  real alpha_pos_mean; // Learning rate (reward/correct; log odds)
  real alpha_pos_sd;
  real alpha_neg_mean; // Learning rate (punishment/incorrect; log odds)
  real alpha_neg_sd;
  real<lower=0> beta_alpha; // Inverse temperature, choice determinism (0-inf)
...
parameters {
  ...
  real alpha_pos; // Learning rate (positive feedback), reward learning rate
  real alpha_neg; // Learning rate (negative feedback), punishment learning rate
  real<lower=0.0000001> beta; // Inverse temperature
  real theta0; // Dimension weight in stage 3 (first stage with 2 dimensions)
...
model {
  ...
  // Priors
  target += normal_lpdf(alpha_pos | alpha_pos_mean, alpha_pos_sd); // Learning rate positive feedback
  target += normal_lpdf(alpha_neg | alpha_neg_mean, alpha_neg_sd); // Learning rate negative feedback
  target += gamma_lpdf(beta | beta_alpha, beta_beta); // Inverse temperature (gamma)
  target += normal_lpdf(theta0 | theta0_mean, theta0_sd); // Initial dimension weights
  ...
  for (t in 1:n_trials) {
    ...
    // Get alpha depending on whether the choice was correct or incorrect
...
generated quantities {
  ...
  array[2,2] int f_ints; // Array containing ints of features defining correct and incorrect stimuli
  vector[4] f; // Vector of feature weights
  vector[4] f_updated; // Vector of updated feature weights after trial
  real alpha; // Learning rate
  real theta; // Dimension weight 
  ...
  for (t in 1:n_trials) {
    ...
    f_updated = update_weights(stage[t], stimulus[t], f, alpha, beta_prior, theta);
    if (stage[t] > 2) {
      theta_updated = update_theta(stage[t], stimulus[t], f, alpha, beta_prior, theta);
    }
  }
