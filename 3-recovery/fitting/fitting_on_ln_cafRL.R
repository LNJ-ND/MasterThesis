# FITTING ON LN-cafFRL DATA WITH BETA FIXED TO 2 #
...
out_path_sim = paste0("output/sim_ln_cafRL")
...
for (i in 1:nrow(param_combination)) {
  ...
  params = list(
    ...
    group = "recovery",
    n_correct = 6,
    alpha_pos = param_combination$alpha_pos[i],
    alpha_neg = param_combination$alpha_neg[i],
  ...
  if (model == "yearsley" & param_combination$n_agent[i] == 1) {
